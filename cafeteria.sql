...
--CRIA AS FUNÇÕES DE ATUALIZAÇÃO E RESTRIÇÃO POR VALORES

      --Diminui a quantidade de itens no estoque a cada pedido (FUNCIONA)
...
EXECUTE FUNCTION atualizar_estoque();
      ...
      --Atualiza a disponibilidade para false se acabar (NAO FUNCIONA)
CREATE OR REPLACE FUNCTION atualizar_disponibilidade()
RETURNS TRIGGER AS $$
BEGIN
  -- Verifica se o estoque é igual a zero
  IF NEW.QuantidadeEmEstoque = 0 THEN
    -- Define a disponibilidade como false
    UPDATE Produtos
...
CREATE TRIGGER atualizar_disponibilidade_trigger
BEFORE UPDATE ON Estoque
FOR EACH ROW
EXECUTE FUNCTION atualizar_disponibilidade();

      --Checa a cada inserção de pedido se há o produto disponível (FUNCIONA)
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM Produtos
    WHERE ID_produto = NEW.ID_produto AND Disponibilidade = true
  ) THEN
    RAISE EXCEPTION 'Produto nao esta disponivel';
  END IF;
...
CREATE TRIGGER verificar_disponibilidade_trigger
...
--Lista os produtos que não estão disponíveis no estoque no momento
...
--Lista os funcionários que possuem algum dependente
SELECT pNome, uNome, n_dependentes
FROM Funcionarios
WHERE n_dependentes > 0;

--Lista todas as avaliações feitas por um cliente específico
SELECT Classificacao, Comentario, DataAvaliacao
FROM AvaliacoesComentarios
WHERE ID_cliente = 157;

--Lista todos os pedidos com o nome dos respectivos clientes
...
--Lista todos os produtos em estoque com seu fornecedor
...
--Lista todos os itens de pedido para um pedido específico com informações sobre o produto
...
--Lista todos os pedidos feitos por uma mesa específica com informações sobre o pedido
...
--A ideia da professora, lista todos os pedidos (nome do produto e quantidade) associados ao nome do cliente
SELECT C.pNome, C.uNome, Pr.NomeProduto, IP.Quantidade
FROM Pedidos P
JOIN Itens_Pedido IP ON P.ID_pedido = IP.ID_pedido
JOIN Produtos Pr ON IP.ID_produto = Pr.ID_produto
JOIN Clientes C ON P.ID_cliente = C.ID_cliente;

--Lista todos os produtos que estão em falta no estoque e seus fornecedores
SELECT Pr.NomeProduto, F.NomeFornecedor
FROM Produtos Pr
JOIN Estoque E ON Pr.ID_produto = E.ID_produto
JOIN Fornecedores F ON E.ID_fornecedor = F.ID_fornecedor
WHERE E.QuantidadeEmEstoque = 0;

--A ideia da professora, lista detalhadamente os pedidos feitos por uma mesa em específico
SELECT P.ID_pedido, Pr.NomeProduto, IP.Quantidade
FROM Pedidos P
JOIN Itens_Pedido IP ON P.ID_pedido = IP.ID_pedido
JOIN Produtos Pr ON IP.ID_produto = Pr.ID_produto
WHERE P.ID_mesa = 3;

--Lista os pedidos ainda em andamento de um cliente em específico
SELECT P.ID_pedido, P.StatusPedido, Pr.NomeProduto, IP.Quantidade
FROM Pedidos P
JOIN Itens_Pedido IP ON P.ID_pedido = IP.ID_pedido
JOIN Produtos Pr ON IP.ID_produto = Pr.ID_produto
WHERE P.ID_funcionario = 181 AND P.StatusPedido = 'Em andamento';

--Elimina um cliente que não está conectado a nenhuma outra tabela, se estiver ele não permite e dá erro
DELETE FROM Clientes WHERE ID_cliente = 070 ;
SELECT * FROM Clientes ;

--Dá um aumento para o funcionário Lucas
